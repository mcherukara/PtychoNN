...
def getIntPositions(
        positions_all: np.ndarray,  # Set as (y,x).
        columnwise:
    bool = False,  # whether the images are column-wise or row-wise. True corresponds to columnwise
    ...
    raise NotImplementedError(
        "I am not sure it is working correctly just yet.")
    if columnwise:
...
def stitch(
        ...
        columnwise:
    bool = False,  # whether the images are column-wise or row-wise
    ...
    size_h = size // 2
    composite = np.zeros(
        (np.max(pos_int_row) + size, np.max(pos_int_col) + size), slices.dtype)
    #print('Composite shape before trimming', composite.shape)
    ctr = np.zeros(composite.shape)
    if weights is None:
        weights = np.ones(
            (np.array(slices[0].shape) * upsample_factor).astype('int32'),
            dtype='float32')
    ...
    for i in range(pos_int_row.shape[0]):
        ...
        if upsample_factor > 1:
            if slice_to_add.dtype in [np.complex64, np.complex128]:
                mag_slice_to_add = skimage.transform.rescale(
                    np.abs(slice_to_add), upsample_factor, preserve_range=True)
                ph_slice_to_add = skimage.transform.rescale(
                    np.angle(slice_to_add),
                    upsample_factor,
                    preserve_range=True)
                slice_to_add = mag_slice_to_add * np.exp(1j * ph_slice_to_add)
            else:
                slice_to_add = skimage.transform.rescale(slice_to_add,
                                                         upsample_factor,
                                                         preserve_range=True)

        composite[pos_int_row[i]:pos_int_row[i] + size,
                  pos_int_col[i]:pos_int_col[i] +
                  size] += slice_to_add * weights

        ctr[pos_int_row[i]:pos_int_row[i] + size,
            pos_int_col[i]:pos_int_col[i] + size] += weights  #pb_weight
...
class Trainer():
    def __init__(self, model: ReconSmallPhaseModel, batch_size: int,
                 output_path: str, output_suffix: str):
        self.model = model
    ...
    def setTrainingData(self,
        ...
        self.iters_per_epoch = int(
            np.floor((self.ntrain) / self.batch_size) +
            1)  #Final batch will be less than batch size

    def setOptimizationParams(self,
    ...
    def updateSavedModel(model: ReconSmallPhaseModel,
                         path: str,
                         output_suffix: str = ''):
